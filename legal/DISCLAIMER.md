...
Package metadata, latest-version information and code-health reports are derived from third-party sources (PyPI, and deptry/vulture/ruff output) and from heuristics such as reverse-dependency analysis. They may be incomplete, outdated, or wrong — for example, dead-code detection produces false positives on dynamically referenced code, and deptry reports a module name rather than the distribution that provides it. **Review before acting**; PipDock's reports are assistance, not authority. The Security tab reports known advisories against your installed packages. Those come from **PyPI's advisory data**, by way of pip-audit, and are covered by this paragraph too: an advisory database is a record of what has been reported, not a guarantee that a package is safe or that an absence of findings means there is nothing to find. Each finding links to its entry on osv.dev so you can read the original. PipDock assigns no severity and no risk score, because pip-audit publishes none.
